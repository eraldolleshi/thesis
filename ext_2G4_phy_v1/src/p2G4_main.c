...
int n =0;
   char *number_str = getenv("SCENARIO"); // Get the value of the environment variable
    if (number_str != NULL) {
...
if (n==1){
 rx_possible_abort_recheck(d, &rx_a[d], false);
  ...
  if ( ( ( current_time >= rx_a[d].header_end )
        && ( rx_a[d].biterrors > 1000 ) )
      ||
       ( current_time >= rx_a[d].rx_s.abort.abort_time ) ) {
   rx_a[d].rx_done_s.status = 3;
...
void introduce_bit_flips(uint8_t *packet, size_t num_bytes, int bitErrors, int error_type) {
    // Determine the number of bits to flip based on BER
    size_t num_bits_to_flip = (size_t)(bitErrors);

    // Generate random indices for flipping bits
    size_t *indices = (size_t *)malloc(num_bits_to_flip * sizeof(size_t));
    for (size_t i = 0; i < num_bits_to_flip; i++) {
        indices[i] =  9*8 + (rand() % ((num_bytes-9-3)* 8));
    }
    
        // Perform the bit flips
    for (size_t i = 0; i < num_bits_to_flip; i++) {
        size_t byte_index = indices[i] / 8;
        size_t bit_index = indices[i] % 8;
...
int n =0;
  ...
  if (((current_time >= rx_a[d].payload_end) && (rx_a[d].biterrors > 1000))
      || (current_time >= rx_a[d].rx_s.abort.abort_time)) {
    if (!args.crcerr_data) {
       rx_a[d].rx_done_s.packet_size = 0;
    }
    rx_a[d].rx_done_s.status = 2;
  ...
  if ( current_time >= rx_a[d].payload_end ) {
  ...
  if (((current_time >= rx_a[d].payload_end) && (rx_a[d].biterrors > 100))
      || (current_time >= rx_a[d].rx_s.abort.abort_time)) {
    if (!args.crcerr_data) {
       rx_a[d].rx_done_s.packet_size = 0;
    }
  ...
  if ( current_time >= rx_a[d].payload_end ) {
    ...
    size_t num_bytes = (size_t)rx_a[d].rx_done_s.packet_size;
    int n1 =0;
   char *number_str = getenv("NUMBER"); // Get the value of the environment variable
    if (number_str != NULL) {
        ...
        int error_type =0;
   char *number1_str = getenv("ERROR_TYPE"); // Get the value of the environment variable
    if (number1_str != NULL) {
        int number1 = atoi(number1_str); // Convert string to integer
        error_type=number1;
        }
    if (num_bytes == n1+12){
    introduce_bit_flips(tx_l_c.tx_list[rx_a[d].tx_nbr].packet, num_bytes,rx_a[d].biterrors, error_type);
}    
    dump_rx(&rx_a[d],tx_l_c.tx_list[rx_a[d].tx_nbr].packet,d);
    p2G4_handle_next_request(d);
    return;
