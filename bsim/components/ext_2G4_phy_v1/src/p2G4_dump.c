...
//if (packet==NULL){
  ...
  if ( ( resp->packet_size > 0 ) && ( packet != NULL ) ) {
   ...
   char *number_str = getenv("NUMBER"); // Get the value of the environment variable
    if (number_str != NULL) {
     ...
     int sizee =0;
   char *size_str = getenv("SIZE"); // Get the value of the environment variable
    if (size_str != NULL) {
        int number = atoi(size_str); // Convert string to integer
        sizee=number;
    }
    int m = sizee%n;
    
  if (resp->packet_size==n+12 || resp->packet_size==m+12){
  int number_bytes=0;
  
  
  if (resp->packet_size==n+12){
  number_bytes=n;
  }
  else if (resp->packet_size==m+12){
  number_bytes=m;
  
  }
 
  int bytes_as_int[number_bytes]; 
   int filler_value =0;
   char *number_str = getenv("FILLER_VALUE"); // Get the value of the environment variable
    if (number_str != NULL) {
        int number = atoi(number_str); // Convert string to integer
        filler_value = number;
        }
  ...
  if (packetstr[0] == '\0'){
   for (int i = 0; i < number_bytes; i++) {
        bytes_as_int[i] = filler_value; //replacing the values of the lost bytes
  }
  }
  else{
  int start_index = 27;
    int end_index = strlen(packetstr) - 9;

    if (start_index < end_index && start_index >= 0) {
        memmove(packetstr, packetstr + start_index, end_index - start_index + 1);
        packetstr[end_index - start_index + 1] = '\0';
    } 
    ...
    while (token != NULL) {
        bytes_as_int[i++] = (int)strtol(token, NULL, 16); 
        token = strtok(NULL, " "); 
    }
    }
    ...
    FILE *binary_file = fopen(output_path, "ab");
    

    
    FILE *csv_file = fopen("simulator_output.csv", "a");
...
if ((number_bytes != m) || ((ftell(csv_file) != 0) &&(ftell(binary_file))) ) {
    

    for (int i = 0; i < number_bytes; i++) {
        unsigned char byte = (unsigned char)bytes_as_int[i];
        fwrite(&byte, sizeof(unsigned char), 1, binary_file);

        if (i > 0) {
            // If not the first value, add a comma in the CSV file
            fprintf(csv_file, ",");
        }

        // Write the value to the CSV file
        fprintf(csv_file, "%d", bytes_as_int[i]);
    }

    // Add a new line in the CSV file for the next set of values
    fprintf(csv_file, "\n");
